...
public class MemberDAO {
	...
	public int userCheck(String id,String passwd){
		...
		return check; // 1 또는 0 또는 -1 를 리턴 //loginPro.jsp로 리턴
	...
	//check변숙밧을 0으로 저장 하여<-----아이디 중복 아님을 나타내고....
	...
	public int idCheck(String id){
		...
		try {
			...
			//?기호에 대응되는 SELECT값을 제외한 나머지 전체 SELECT문장을
			//PreparedStatement객체에 담아 반환 받기
			pstmt = con.prepareStatement(sql);
			...
			//검색!!!! 검색후 그결과를 ResultSet에 담아 반환
			rs = pstmt.executeQuery();
			
			//6. 우리가 입력한 아이디에 해당하는 레코드가 검색 되었다면?(id가 존재 하면,id가중복되었다면)
			if(rs.next()){
				check = 1;
			}else{//입력한 아이디에 해당하는 회원레코드가 검색 되지 않으면?
				  //(id가 중복 되지 않았다면)
				check = 0;
			}
		} catch (Exception e) {
