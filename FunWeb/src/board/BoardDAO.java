...
public class BoardDAO {
	...
	/*
	  ...
	  re_lev -> 같은 그룹내에서의 깊이(들여쓰기정도 레벨)
	  
	  답변 달기 규칙 설명
	  순서1) re_ref값은 부모글(주글)의 그룹번호(re_ref)를 사용 해야 함.
	  순서2) re_seq 값은 부모글의 re_seq에서  + 1증가 한값을 사용 해야 함.
	  순서3) re_lev 값은  부모글의 re_lev에서 ... + 1증가 한 값을 사용 해야 함.
	 ...
	 */
	 ...
	 public void reInsertBoard(BoardBean bBean){		 
		 ...
		 try {
			 ...
			 pstmt.setString(11, bBean.getIp());//답변글을 작성 하는 사람의 IP주소 		 
			 //답변 INSERT
			 pstmt.executeUpdate();	 
		 } catch (Exception e) {
			System.out.println("reInsertBoard메소드 오류 : " + e);
		 } finally {
			//자원해제
	...
	//글수정 화면에서 입려한 비밀번호가 일치하면? 글 정보 UPDATE!!!!
