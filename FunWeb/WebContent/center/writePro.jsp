...
// 	액션 태그란? jsp페이지내에서 자바코드와 html디자인이 섞여 있으므로 복잡하다..
...
%>
	...
	<%--request객체에서 꺼내온 모든값을? 
		BoardBean객체의 모든 setter메소드 호출 하여 모든 변수에 저장 --%>
	<jsp:setProperty property="*" name="bBean"/>
	<%--
		setProperty액션태그를 사용 하여
		request에서 꺼내온값을 한꺼번에 BoardBean에 저장 하기 위한 조건
	 	->BoardBean클래스의 각변수이름과  
	 	  write.jsp페이지의 각 <input>태그의 name속성값이 동일 해야함
	 --%>
<%
	//1.2.1 현재 글쓴 날짜, 시간 정보....를 추가로 BoradBean에 저장
	bBean.setDate(new Timestamp(System.currentTimeMillis()));
	//1.2.2 글쓴사람의 ip주소를 추가로 BoradBean에 저장
