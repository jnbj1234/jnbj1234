...
<title>Insert title here</title>
	...
	<script type="text/javascript">
		function result() {
// 			사용가능한 ID이면.. 사용함 버튼을 눌러..부모창인?join.jsp에 
...
<body>
	<%
		/*요청값 받기*/
		//1.join.jsp의 function winopen()함수에 의해서 전달 받은 userid값 얻기
		//2. 아래의 중복확인 버튼 클릭시..<form>태그로 부터 전달받은 userid값 얻기
		String id = request.getParameter("userid");
		
		/*요청값에 의해 DB작업(DB에 저장된  ID와 우리가 입력한 ID가 동일 한지 체크)*/
		MemberDAO mdao = new MemberDAO();
		
		//3.입력한 id값을 MemberDAO객체의 idCheck()메소드호출시...
		//매개변수로 전달하여 DB에 저장된 ID와 중복되는지  유무 얻기
		int check = mdao.idCheck(id); //check=1 아이디중복
									  //check=0 아이디중복 아님
